...
int guac_user_handle_connection(guac_user* user, int usec_timeout) {
    ...
    if (guac_client_add_user(client, user, (parser->argc - 1), parser->argv + 1))
        guac_client_log(client, GUAC_LOG_ERROR, "User \"%s\" (%s) could NOT "
                "join connection \"%s\"", user->user_id, user->info.name, client->connection_id);
    ...
    /* Begin user connection if join successful */
        ...
        guac_client_log(client, GUAC_LOG_INFO, "User \"%s\" (%s) disconnected (%i "
                "users remain)", user->user_id, user->info.name, client->connected_users);
